    ...
    <AppLayout :title="circle.name">
        ...
        <div class="mt-2 bg-white dark:bg-sand-900">
            <div class="flex items-center justify-between border-b border-sand-100 px-4 py-3 dark:border-sand-800">
                ...
                <button class="text-sage-600 dark:text-sage-400" @click="showAddMember = !showAddMember">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-5">
            ...
            <!-- Empty Members State -->
            <div v-if="circle.members.length === 0" class="px-4 py-8 text-center">
                <p class="text-sm font-medium text-sand-600 dark:text-sand-300">{{ t('No members yet') }}</p>
                <p class="mt-1 text-sm text-sand-400 dark:text-sand-500">{{ t('Add people by their username.') }}</p>
            </div>
        </div>
    </AppLayout>
</template>
