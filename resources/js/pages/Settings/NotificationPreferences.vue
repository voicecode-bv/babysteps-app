...
const localPreferences = ref<NotificationPreferences | null>(props.preferences ?? null);
...
const labels = computed<Record<keyof NotificationPreferences, string>>(() => ({
...
function togglePreference(key: keyof NotificationPreferences) {
    ...
    router.put('/settings/notifications', localPreferences.value, {
        preserveScroll: true,
    });
}
</script>
            ...
            <div class="relative space-y-4 px-4 pt-4 pb-24">
                <SurfaceCard>
                    ...
                    <Deferred data="preferences">
                        ...
                        <ul v-if="localPreferences" class="mt-3 divide-y divide-sand-100 dark:divide-sand-700/60">
                            <li v-for="(label, key) in labels" :key="key">
                                <label class="flex cursor-pointer items-center justify-between gap-3 py-3">
                                    ...
                                    <button
                                        type="button"
                                        role="switch"
                                        :aria-checked="localPreferences[key]"
                                        :class="localPreferences[key] ? 'bg-teal' : 'bg-sand-300 dark:bg-sand-600'"
                                    ...
                                    >
                                        <span
                                            :class="localPreferences[key] ? 'translate-x-7' : 'translate-x-1'"
