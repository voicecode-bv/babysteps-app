...
async function submit(): Promise<void> {
    await form.post<{ data: { id: number; name: string } }>('/circles', {
        onSuccess: (response) => {
            router.push({ name: 'spa.onboarding.invite-members', params: { circle: response.data.id } });
        },
    });
}
                ...
                <div>
                    ...
                    <div class="flex flex-wrap gap-2">
                        <button
                            ...
                            :class="form.data.name === suggestion ? 'border-teal/40 bg-sage-100 text-teal dark:border-teal/40 dark:bg-sage-900/40 dark:text-sage-100' : ''"
