...
onMounted(async () => {
    try {
        const data = await externalApi.get<{ data: Circle }>(`/circles/${circleId}`);
...
function friendlyApiError(error: ApiError, field: 'email' | 'username'): string {
...
async function submit(): Promise<void> {
    ...
    try {
        ...
        if (error instanceof ApiError && error.status === 429) {
            form.errors = { identifier: t('Too many invitations sent. Please try again later.') };
                ...
                <form class="relative rounded-lg bg-white/50 p-5 shadow-sm backdrop-blur-sm dark:border-sand-700/50 dark:bg-sand-800/60" @submit.prevent="submit">
                    ...
                    <div class="mt-3 flex items-center gap-3">
                        ...
                        <button
                            type="submit"
                            :aria-label="t('Add')"
                            :disabled="!form.data.identifier.trim() || form.processing"
